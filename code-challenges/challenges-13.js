...
const objectCounter = (obj) => {
    ...
    const couObj = obj.reduce((accumulator, currentValue, index)=>{ 
    index = index+1;
    return index;
      },0)
    return couObj;
...
const stringReverse = (str) => {
    ...
    let revArr = [];
    const proArr = splitStr.reduce((accumulator, currentValue, index)=>{
       
        revArr.unshift(currentValue);

    },0)
  let nonSplitStr= revArr.join(' ')
  return nonSplitStr;
...
const statistics = (obj) => {
    return obj.reduce((accumulator, currentValue, index) => {
