...
const cvFormatter = (arr) => {
    ...
    let arr2 = [];
        for (let i = 0; i < arr.length; i++) {
                 const object = arr[i];
            if (object.lastName === null && object.yearsOfExperience > 1) {
                     arr2.push({ fullName: `${object.firstName}`, tech: `${object.tech}` });
    
            } else if (object.yearsOfExperience > 1) {
                         arr2.push({ fullName: `${object.firstName} ${object.lastName}`, tech: `${object.tech}` });
            } else continue; 
        } return arr2;
...
const classesAvg = (data) => {
    for (let i =0; i < data.grades.length; i++) {
        ...
        for (let j =0; j < grade.classes.length; j++) {
            let _class = grade.classes[j];
            let sum = _class.classScores.reduce((a,b) => a + b);
            let avg = Math.floor(sum / _class.classScores.length);
            _class.avg = avg;
        }
    }
    return data;
