...
function gradesAvg(arr) {
    let grAvg = arr.map((item)=>{
    let gradsL = item.gradsList;
    let sum =gradsL.reduce((a, b) => a + b);
    let avg = sum / gradsL.length; 
    item.avg = avg;
    return(item);
    });
  return(grAvg);
